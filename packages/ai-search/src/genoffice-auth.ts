/**
 * Captivela Office's own hosted service identity: device-code login (office_addin_auth,
 ...
 * subprocesses) to Captivela Office. Stored in ~/.genoffice/auth.json, deliberately
...
/** The Captivela Office-named api key; '' when not signed in. Cached (invalidated by login/logout). */
...
/**
 * Starts the Captivela Office device-code login, cancelling a previous in-flight one
...
/**
 * Signs out of Captivela Office only: best-effort server-side revoke of the
